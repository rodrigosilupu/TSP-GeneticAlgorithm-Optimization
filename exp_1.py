...
class GeneticAlgorithmTSP:
    ...
    def select(self, population, fitnesses):
        if self.selection_method == "roulette":
            ...
            rank_prob = [1.0 / (i+1) for i in range(len(rank))]
            total_rank_prob = sum(rank_prob)
            selection_probs = [rp / total_rank_prob for rp in rank_prob]
            selected_index = np.random.choice(range(len(population)), p=selection_probs)
...
def main():
    output_dir = 'results_experiment1'
    os.makedirs(output_dir, exist_ok=True)
    
    # Define the cities as a list of coordinates
    cities = [(random.random(), random.random()) for _ in range(100)]
    
    methods = ["roulette", "rank", "tournament"]
    histories = []
    best_individuals = {}
    
    for method in methods:
        print(f"Running Genetic Algorithm with {method} selection")
        ga = GeneticAlgorithmTSP(cities, population_size=100, generations=3000, mutation_rate=0.01, selection_method=method, init_method="random")
